passwords = ["13-15 c: cqbhncccjsncqcc",
             "2-3 v: zvdvfd",
...
for x in passwords:
    y = x.split()
    policy_range.append(y[0])
    policy_type.append(y[1])
    policy_password.append(y[2])

for minus in range(len(policy_range)):
    chunk = policy_range[minus].split("-")
    chunk = list(map(int, chunk))
    policy_range[minus] = chunk

for dots in range(len(policy_type)):
    policy_type[dots] = policy_type[dots].replace(":", "")

# sorting the received input in 3 seperate lists


correct_passwords_part_1 = 0
correct_passwords_part_2 = 0
# part 1
for words in range(len(policy_password)):
    local_range = range(policy_range[words][0], policy_range[words][1] + 1)
    local_type = policy_type[words]
    if policy_password[words].count(local_type) in local_range:
        correct_passwords_part_1 += 1
    else:
        continue
print(f"Correct passwords of part 1: {correct_passwords_part_1}")
# counting the amount of times policy letter is in the password and comparing it with the policy range


# part 2
for words in range(len(policy_password)):
    local_index = [policy_range[words][0] - 1, policy_range[words][1] - 1]
    local_type = policy_type[words]
    local_word = policy_password[words]
    if local_word[local_index[0]] == local_type and local_word[local_index[1]] == local_type:
        continue
    elif local_word[local_index[0]] != local_type and local_word[local_index[1]] != local_type:
        continue
    else:
        correct_passwords_part_2 += 1
print(f"Correct passwords of part 2: {correct_passwords_part_2}")
# checking if letter is only at one index position
# - 1 on both local index, because they dont believe in index 0 (first index is 1 for them)
# if letter is at both or none, it's invalid, so everything else is valid
